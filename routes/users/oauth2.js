...
const vpnCheck = require("../functions/vpnCheck");
        ...
        if ((newsettings.api.client.oauth2.ip["duplicate check"] == true) && ip !== '127.0.0.1') {
          ...
          if (ipuser && ipuser !== userinfo.id) {
            renderFile(
              ...
              null,
              (err, str) => {
                if (err) return res.send('Another account on your IP has been detected, there can only be one account per IP. Think this is a mistake? <a href="https://discord.gg/pTd99nhWRn" target="_blank">Join our discord.</a>')
                res.status(200);
                res.send(str);
              }
            )
