...
module.exports.load = async function(app, db) {
    ...
    app.get("/getip", async (req, res) => {
        ...
        return res.redirect(successredirect + "?err=IPFOUND&ip=" + ip)
    });
    ...
    app.get("/purge", async (req, res) => {
        ...
        try {
         ...
         let failredirect = theme.settings.redirect.failedpurge
         let successredirect = theme.settings.redirect.successpurge
          if (!response.ok) {
            location.href = failredirect + '?err=FAILEDPURGE'
          ...
          const inactiveServers = servers.filter(
            (server) => !server.attributes.name.includes(req.query.keyword)
          );
          ...
          location.href = successredirect + '?err=SUCCESSFULLYPURGED' + '?keyword=' + req.session.keyword
          log(`purge`, `${req.session.userinfo.username}#${req.session.userinfo.discriminator} Purged all the servers without the keyword - \`${req.query.keyword}\`.`)
          res.sendStatus(200);
        } catch (error) {
