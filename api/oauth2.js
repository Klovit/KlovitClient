...
.loadingbar .bar {
  ...
  app.get(`/submitlogin`, async (req, res) => {
    ...
    if (newsettings.api.antivpn.status && ip !== '127.0.0.1' && !newsettings.antivpn.whitelistedIPs.includes(ip)) {
    ...
    if (json.ok == true) {
      ...
      if (settings.api.whitelist.status) {
        if (!settings.api.whitelist.users.includes(userinfo.id)) return res.send('Service is under maintenance.')
      }
        ...
        if ((newsettings.api.client.oauth2.ip["duplicate check"] == true) && ip !== '127.0.0.1') {
          ...
          if (ipuser && ipuser !== userinfo.id) {
            renderFile(
              ...
              null,
              (err, str) => {
                if (err) return res.send('Another account on your IP has been detected, there can only be one account per IP. Think this is a mistake? <a href="<%= settings.discord %>" target="_blank">Join our discord.</a>')
                res.status(200);
                res.send(str);
              }
            )
