...
module.exports.load = async function (app, db) {
  ...
  app.get("/create", async (req, res) => {
    ...
    if (newsettings.api.client.allow.server.create == true) {
      queue.addJob(async (cb) => {
        ...
        if (!captcha) {
          cb()
          return res.redirect(`/dashboard/new?err=MUSTCOMPLETECAPTCHA`)
        }
        const verified = await verifyCaptchaResponse(captcha)
        if (!verified) {
          cb()
          return res.redirect(`/dashboard/new?err=INVALIDCAPTCHARESPONSE`)
        }
  ...
  app.get("/modify", async (req, res) => {
    ...
    if (newsettings.api.client.allow.server.modify == true) {
      ...
      const captcha = req.query.captcha
      if (!captcha) {
        return res.redirect(`/dashboard/edit?id=${req.query.id}&err=MUSTCOMPLETECAPTCHA`)
      }
      const verified = await verifyCaptchaResponse(captcha)
      if (!verified) {
        return res.redirect(`/dashboard/edit?id=${req.query.id}&err=INVALIDCAPTCHARESPONSE`)
      }
