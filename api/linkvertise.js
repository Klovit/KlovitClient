...
module.exports.load = async function (app, db) {
    ...
    app.get(`/earn/lv/gen`, async (req, res) => {
        ...
        if (!captcha) return res.redirect(`/earn?err=MUSTCOMPLETECAPTCHA`)
        const verified = await verifyCaptchaResponse(captcha)
        if (!verified) return res.redirect(`/earn?err=INVALIDCAPTCHARESPONSE`)

        let referer = req.headers.referer
        if (!referer) return res.send('An error occured with your browser!')
        referer = referer.toLowerCase()
        if (referer.includes('?')) referer = referer.split('?')[0]
        if (!referer.endsWith(`/earn`) && !referer.endsWith(`/earn/`)) return res.send('An error occured with your browser!')
    ...
    app.get(`/earn/redeem`, async (req, res) => {
