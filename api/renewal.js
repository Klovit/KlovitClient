...
module.exports.load = async function (app, db) {
    ...
    app.get(`/api/renewalstatus`, async (req, res) => {
        if (!settings.api.renewals.status) return res.json({ error: true })
        ...
        if (lastRenew > Date.now()) return res.json({ text: 'Renewed', success: true })
            ...
            if ((Date.now() - lastRenew) > (settings.api.renewals.delay * 86400000)) {
                return res.json({ text: 'Last chance to renew!', renewable: true })
            }
            const time = msToDaysAndHours((settings.api.renewals.delay * 86400000) - (Date.now() - lastRenew))
            return res.json({ text: time, renewable: true })
        }
    })

    app.get(`/renew`, async (req, res) => {
        if (!settings.api.renewals.status) return res.send(`Renewals are currently disabled.`)
        ...
        const lastRenew = await db.get(`lastrenewal-${req.query.id}`)
        if (!lastRenew) return res.send('Renewed already')
        ...
        if (settings.api.renewals.cost > coins) return res.redirect(`/dashboard` + "?err=CANNOTAFFORDRENEWAL")

        await db.set("coins-" + req.session.userinfo.id, coins - settings.api.renewals.cost)

        const newTime = lastRenew + (settings.api.renewals.delay * 86400000)
    ...
    new CronJob(`0 0 * * *`, () => {
        if (settings.api.renewals.status) {
